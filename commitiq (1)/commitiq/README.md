...
Other commands:
```bash
git commitiq ui           # launch the React + Electron Visual Commit Note Graph for current repo
...
## Visual Commit Note Graph UI (Electron + React)

CommitIQ features an interactive desktop UI inspired by vertical connected stream visualization ("Historia de la Música Visual").

To launch the desktop app for the current repository:
```bash
git commitiq ui
```

Features:
- **Connected Commit Stream Graph**: Color-coded branch tracks, curved Bezier parent-child connections, and vertical date scale axis.
- **CommitIQ Note Callout Cards**: Displays conventional commit badges (`feat`, `fix`, `refactor`, `docs`, `breaking`), scopes, summaries, and review notes attached to nodes.
- **Interactive Commit Drawer & Diff Viewer**: Click any commit to view full JSON notes, reviewer notes, breaking changes, changed file lists, and raw syntax-highlighted git diffs.
- **Live CommitIQ Commit Trigger**: Create new commits with automated LLM summary generation directly from the UI.
- **Scoped to Repository**: Displays the connected commit topology exclusively for the repo initialized in the working directory.
